...
*/
function sup_getBalanceSheet(){
  var rr = sup_getRangeValsByName('RANGEGSTBALANCESHEET');
  if(!rr) return [];
  var vals = rr.values;
...
*/
function sup_addBalanceAsset(obj){
...
function sup_updateBalanceByRow(sheetRow, updatedObj){
  var rr = sup_getRangeValsByName('RANGEBALANCESHEET');
  if(!rr) return { status:'error', message:'RANGEBALANCESHEET not found' };
  var headers = rr.values[0].map(function(h){ return String(h||'').trim(); });
  var sheet = rr.sheet;
...
function sup_deleteBalanceByRow(sheetRow){
  var rr = sup_getRangeValsByName('RANGEBALANCESHEET');
  if(!rr) return { status:'error', message:'RANGEBALANCESHEET not found' };
  var sheet = rr.sheet;
  try {
...
function sup_exportBalancePdf(rows, meta){
